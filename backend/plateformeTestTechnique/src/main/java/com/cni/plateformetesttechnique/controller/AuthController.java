...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
                ...
                switch (role.toLowerCase()) {
                    case "admin":
                        Role adminRole = roleRepository.findByName(ERole.ROLE_ADMIN)
                                .orElseThrow(() -> new RuntimeException("Error: Role not found."));
                        roles.add(adminRole);
                        user.setActive(true);  // L'admin est toujours activé par défaut
        ...
        if (strRoles.contains("developpeur")) {
            ...
            administrateur.setActive(true); // Admin est toujours activé par défaut
            userRepository.save(administrateur);
        } else {
            // Par défaut, on crée un utilisateur normal avec un rôle de base
            user.setActive(true);
...
//            user.setActive(true);
//        }
// Création de l'utilisateur en fonction de son rôle
